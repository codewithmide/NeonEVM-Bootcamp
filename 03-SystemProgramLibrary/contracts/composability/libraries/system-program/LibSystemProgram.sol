...
library LibSystemProgram {
        ...
        )
    {
        ...
        accounts[0] = payer;
        accounts[1] = LibSystemData.getCreateWithSeedAccount(
            basePubKey,
            programId,
            seed
        ...
        bytes8 seedLenLE = bytes8(
            SolanaDataConverterLib.readLittleEndianUnsigned64(
                uint64(seed.length)
            )
        );
        bytes8 rentExemptionBalanceLE = bytes8(
            SolanaDataConverterLib.readLittleEndianUnsigned64(
                rentExemptionBalance
            )
        );
        bytes8 accountSizeLE = bytes8(
            SolanaDataConverterLib.readLittleEndianUnsigned64(accountSize)
        );
        ...
        )
    {
        ...
        bytes8 amountLE = bytes8(
            SolanaDataConverterLib.readLittleEndianUnsigned64(amount)
        );
        data = abi.encodePacked(
    ...
    {
        accounts = new bytes32[](2);
        accounts[0] = LibSystemData.getCreateWithSeedAccount(
            basePubKey,
            programId,
            seed
     ...
     * @dev Wrapper around LibSystemData.getAccountInfo that returns all key account properties in a single struct
     * @param accountPubKey The 32-byte Solana account public key to query
     * @return AccountInfo struct containing:
     *         - pubkey: The account's public key
     *         - lamports: Current balance in lamports
     *         - owner: 32-byte public key of the owning program
     *         - executable: Boolean indicating if account contains executable program
     *         - rent_epoch: The epoch when rent was last collected
     * @custom:security This is a view function that only reads account data
     */
    function getAccountInfo(
        bytes32 accountPubKey
    ) public view returns (LibSystemData.AccountInfo memory) {
        return LibSystemData.getAccountInfo(accountPubKey);
    }

    /**
     * @notice Validates multiple properties of a Solana account against expected values
     * @dev Performs multiple checks in a single transaction, reverting if any check fails.
     *      Useful for pre-condition checks before account operations.
     * @param accountPubKey The 32-byte Solana account public key to validate
     * @param expectedOwner The 32-byte program ID that should own the account
     * @param minBalance Minimum required lamport balance (inclusive)
     * @param expectedExecutable Expected executable state (true for program accounts)
     * @return true if all validations pass (reverts otherwise)
     * @custom:reverts With descriptive message if any validation fails:
     *         - "Account owner mismatch" if owner doesn't match
     *         - "Insufficient account balance" if balance < minBalance
     *         - "Executable flag mismatch" if executable state doesn't match
     *         - "Account is not rent exempt" if rent_epoch < max_uint64
     * @custom:security This is a view function that only reads account data
     */
